...
The supplied `claimPolicy` is binding evidence for factual restraint. Inferred offerings are private creative hypotheses, not public services. Qualifiers such as "may be available" and "where available" do not make an unknown claim safe. In `guidance-only` mode, category advice and local anchor actions such as "Explore Ideas" are appropriate, while business attributed operations and unsupported transactional copy are not. Treat any inferred offering presented as a service, any unsupported business operation, or any forbidden transactional topic as a factual restraint failure.
