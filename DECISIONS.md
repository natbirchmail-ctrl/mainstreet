...
- Attempt 13: the final public audit found literal synthetic paths, credential-shaped test values, stale deployment commit pointers, and a key backup inside ignored repository recovery. Test values now compose at runtime, the release scanner detects credential families without echoing values, every example points to a commit containing all six exact deployed files, and the key backup moved intact to external recovery trash. Six unpushed commits were rebuilt with the same final tree and preserved authors, messages, and timestamps; the original chain remains recoverable from a verified ignored bundle.
